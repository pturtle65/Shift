...
namespace Shift.DataLayer
{
    ...
    public class JobDALSql : IJobDAL
    {
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                if (isSync)
                {
                    using (var tran = connection.BeginTransaction())
                    {
                        try
                        {
                            jobID = connection.Query<string>(query, job, tran).SingleOrDefault();
                            tran.Commit();
                        }
                        catch
                        {
                ...
                else
                {
                    using (var tran = connection.BeginTransaction())
                    {
                        try
                        {
                            var taskResult = await connection.QueryAsync<string>(query, job, tran);
        ...
        private async Task<int> SetCommandStopAsync(ICollection<string> jobIDs, bool isSync)
        {
            ...
            int retValue = 0;
            if (jobIDs.Count == 0) return retValue;
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var tran = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (isSync)
                        {
                            retValue = connection.Execute(sql, new { command = JobCommand.Stop, ids = jobIDs.ToArray(), status = JobStatus.Running }, tran);
                        }
                        else
                        {
                            retValue = await connection.ExecuteAsync(sql, new { command = JobCommand.Stop, ids = jobIDs.ToArray(), status = JobStatus.Running }, tran);
                        }
                        tran.Commit();
                        connection.Close();
                        return retValue;
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var tran = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (isSync)
                        {
                            retValue = connection.Execute(sql, new { command = JobCommand.RunNow, ids = jobIDs.ToArray() }, tran);
                        }
                        else
                        {
                            retValue = await connection.ExecuteAsync(sql, new { command = JobCommand.RunNow, ids = jobIDs.ToArray() }, tran);
                        }
                        tran.Commit();
                        connection.Close();
                        return retValue;
        ...
        {
            int count = 0;
            if (jobIDs.Count == 0) return count;
                ...
                if (notRunning.Count > 0)
                {
                    using (var trn = connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            SET [Percent] = NULL, Note = NULL, Data = NULL
                            WHERE JobID IN @ids; ";
                            if (isSync)
                            {
        ...
        private async Task<int> DeleteAsync(int hours, ICollection<JobStatus?> statusList, bool isSync)
        {
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                //TODO: Does this need to have the await keyword in front of it?
                //await DeleteCachedProgressAsync(deleteIDs); 
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var tran = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (isSync)
                        {
                            count = connection.Execute(sql, new { status = JobStatus.Stopped, ids = jobIDs.ToArray() }, tran);
                        }
                        else
                        {
                            count = await connection.ExecuteAsync(sql, new { status = JobStatus.Stopped, ids = jobIDs.ToArray() }, tran);
                        }
                        tran.Commit();
                        connection.Close();
                    }
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var tran = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (isSync)
                        {
                            count = connection.Execute(sql, new { status = JobStatus.Running, start = DateTime.Now, jobID, processID }, tran);
                        }
                        else
                        {
                            count = await connection.ExecuteAsync(sql, new { status = JobStatus.Running, start = DateTime.Now, jobID, processID }, tran);
                        }
                        tran.Commit();
                        connection.Close();
                    }
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var tran = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (isSync)
                        {
                            count = connection.Execute(sql, new { error, status = JobStatus.Error, jobID, processID }, tran);
                        }
                        else
                        {
                            count = await connection.ExecuteAsync(sql, new { error, status = JobStatus.Error, jobID, processID }, tran);
                        }
                        tran.Commit();
                        connection.Close();
                    }
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                using (var tran = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (isSync)
                        {
                            count = connection.Execute(sql, new { status = JobStatus.Completed, end = DateTime.Now, jobID, processID }, tran);
                        }
                        else
                        {
                            count = await connection.ExecuteAsync(sql, new { status = JobStatus.Completed, end = DateTime.Now, jobID, processID }, tran);
                        }
                        tran.Commit();
                        connection.Close();
                    }
        ...
        private async Task<int> CountRunningJobsAsync(string processID, bool isSync)
        {
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                else
                {
                    ...
                    runningCount = taskResult.FirstOrDefault();
                }
                connection.Close();
            }
                ...
                foreach (var job in jobList)
                {
                    ...
                    using (var tran = connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            if (isSync)
                            {
                                count = connection.Execute(sql, new { processID, job.JobID }, tran);
                            }
                            else
                            {
                                count = await connection.ExecuteAsync(sql, new { processID, job.JobID }, tran);
                            }
                ...
                using (var tran = connection.BeginTransaction())
                {
                    try
                    {
                        if (found == 0)
                        {
                            ...
                            if (isSync)
                            {
                                count = connection.Execute(sql, new { jobID, percent, note, data }, tran);
                            }
                            else
                            {
                                count = await connection.ExecuteAsync(sql, new { jobID, percent, note, data }, tran);
                            }
                        }
                        else
                        {
                            ...
                            if (isSync)
                            {
                                count = connection.Execute(sql, new { percent, note, data, jobID }, tran);
                            }
                            else
                            {
                                count = await connection.ExecuteAsync(sql, new { percent, note, data, jobID }, tran);
                            }
                        }
                        tran.Commit();
                        connection.Close();
